...
#define Tipo ElementoC

typedef struct DequeDE
{
    Tipo ** d;
    int qtd;
    int front;
...
DequeDE * criaDequeDE(int tam){ // retorna um poteiro do tipo Fila
    DequeDE *deq = malloc(sizeof(DequeDE)); // cria o ponteiro da Fila
...
void pushBackE(DequeDE *deq, Tipo * val){
    if (deq!= NULL){ 
...
void pushFrontE(DequeDE *deq, Tipo * val){
